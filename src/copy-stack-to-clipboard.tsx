...
export default async function CopyStackToClipboardCommand() {
  try {
    // Try to get preferences, with fallback to false if not available
    let autoClearOnDrop = false;
    let closeWindowOnDrop = true;
    ...
    try {
      ...
      autoClearOnDrop = prefs.autoClearOnDrop ?? false;
      closeWindowOnDrop = prefs.closeWindowOnDrop ?? true;
    } catch {
      // Preferences not available, defaults are already set
    }
    ...
    // Copy files to clipboard first
    await copyFilesToClipboard(validFiles);

    // NOW handle auto-clear
    if (autoClearOnDrop) {
      // Clear the stack
      await LocalStorage.setItem(STORAGE_KEY, JSON.stringify([]));
      
      // Double-check it was cleared
      const checkRaw = await LocalStorage.getItem<string>(STORAGE_KEY);
      const checkStack = checkRaw ? JSON.parse(checkRaw) : [];
      
      if (checkStack.length === 0) {
        await showToast({
          style: Toast.Style.Success,
          title: "✨ Stack CLEARED!",
          message: `${validFiles.length} files copied | Stack is now EMPTY`,
        });
      } else {
        await showToast({
          style: Toast.Style.Animated,
          title: "⚠️ Clear may have failed",
          message: `${validFiles.length} files copied | Stack still has ${checkStack.length} items`,
        });
      }
    } else {
      await showToast({
        style: Toast.Style.Success,
        title: "📋 Files copied",
        message: `${validFiles.length} files ready | Stack kept (${stack.length} items remain)`,
      });
    }

    // Close window if preference is enabled
